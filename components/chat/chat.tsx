...
import { sendData } from "../../services/microservices";
...
const Chat = (props) => {
  ...
  const getChatHeight = () =>
    styleProps.HeaderHeight + howCanIHelpYouHeight() + ExtraPadding;

  const handleCypherBlockClick = (
            ...
            <ListItem divider sx={{ width: "100%" }}>
                    ...
                    secondary={
                      <React.Fragment>
                        <Typography
                          ...
                          color="text.primary"
                        >
                          Hey there! NeoConverse uses generative AI to help you
                          communicate with neo4j database using natural
                          language. If you encounter any inaccurate responses,
                          please report them using the report icon. Let's start
                          chatting!
                        </Typography>{" "}
                      </React.Fragment>
                    }
              ...
              {m.author.name === "ai" && i !== 0 && (
                <Stack>
                  ...
                  <CodeSharpIcon
                    id={"csi" + i + m.date.toUTCString()}
                    key={"csi" + m.date.toUTCString()}
                    style={{ cursor: "pointer" }}
                    onClick={(e) => handleCypherBlockClick(e, i)}
                  />
                </Stack>
              )}
            </ListItem>
              ...
              {respondWithChart ? (
                <Tooltip title="Respond with Chart">
                  <DonutSmallIcon
                    sx={{ cursor: "pointer" }}
                    onClick={() => {
                      setRespondWithChart(!respondWithChart);
                    }}
                  />
                </Tooltip>
              ) : (
