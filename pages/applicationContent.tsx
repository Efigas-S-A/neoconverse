...
const ApplicationContent: NextPage = () => {
  ...
  const [anchorElShowModel, setAnchorElShowModel] =
    React.useState<HTMLButtonElement | null>(null);
  const [currentDomainImage, setCurrentDomainImage] =
    useState("/realtorchat.png");
  ...
  function extractQuestions(input: string) {
    ...
    return asks?.map((ask, index) => {
      const question = ask
        ?.split("Cypher Query:")[0]
        .trim()
        .replace("Ask:", ""); // Isolate the question
      return {
        question: question,
  ...
  const processDomainChange = async (key: string) => {
    ...
    if (agent) {
      ...
      setOpenAIModel(
        process.env.NEXT_PUBLIC_SMALL_CONTEXT_OPENAI_MODEL as string
      );
  ...
  const StreamResponse = async (e: any) => {
    ...
    let isUserDefined: boolean =
      currentAgent?.userDefined === true ? true : false;

    // TODO: we may want to restrict the total string size of user-defined schemas/fewshots

    track(Events.AskQuestion, {
      saveConvoOn: isUserDefined
        ? currentAgent.saveConvo
        : process.env.NEXT_PUBLIC_SAVE_CONVERSATION,
      isChart: respondWithChart,
    ...
    const filteredMessages = messages.filter(
      (message, index) =>
        index !== 0 && !message.isChart && message.agent == selectedAgentKey
    );
    ...
    const convoHistory = lastTwoMessages
    ...
    // var prompt = initialContext  +' \n'+ fewShotStr+'\n'+additionContext +'\nAsk :'+ userInput+'\nCypher Query:  ';
    // var prompt = initialContext  +' \n'+ fewShotStr+'\n'+ convoHistory +'\nAsk :'+ userInput+'\nCypher Query:  ';
    var prompt = prompts.CYPHER_GENERATION_PROMPT(
      initialContext,
      fewShotStr,
      convoHistory,
      userInput
    ...
    let provider = currentAgent?.userDefined
      ? currentAgent?.aiService
      : "Open AI";

    let responseText = await GenerateContent(
      isUserDefined,
      prompt,
      true,
      respondWithChart,
    ...
    if (responseText.toString().toLowerCase().indexOf("limit") !== -1) {
      ...
      query = respondWithChart
        ? responseText.toString().trim().replace(";", "")
        : responseText.toString().trim().replace(";", "") + "\nLIMIT 5";
    }
    ...
    try {
      ...
      var neoResponse: any = await ExecuteCypher(
        isUserDefined,
        false,
        selectedAgentKey,
        query,
      ...
      if (!respondWithChart) {
        if (neoResponse?.result?.length === 0) {
          ...
          prompt = prompts.HUMAN_READABLE_MESSAGE_PROMPT(
            userInput,
        ...
        if (neoResponse?.result?.length === 0) {
          ...
          prompt = prompts.CHART_GENERATION_PROMPT(
            userInput,
            JSON.stringify(neoResponse.result).trim()
          );
        }
      ...
      let finalResponse = await GenerateContent(
        isUserDefined,
        prompt,
        false,
        respondWithChart,
        ...
        while (true) {
          ...
          !respondWithChart
            ? (userData[userData.length - 1].text =
                userData[userData.length - 1].text + chunkValue)
            : "";
      ...
      if (respondWithChart && neoResponse.result.length != 0) {
        ...
        userData[userData.length - 1].text =
          prompts.GRACEFUL_CHART_FAILURE_PROMPT;
      }
    ...
    let saveConvoCypher = userData[userData.length - 1].cypher.replace(
      /"/g,
      '\\"'
    );
    let saveConvoResposne = userData[userData.length - 1].text.replace(
      /"/g,
      '\\"'
    );
    let cypherQuery = SAVE_CONVO_CYPHER(
      userData[userData.length - 1].conversation_id,
      userInput,
      saveConvoCypher,
      "gpt-4",
      user?.name,
    ...
    if (
      !isUserDefinedAgent &&
      process.env.NEXT_PUBLIC_SAVE_CONVERSATION === "true" &&
      getDomainFromEmail(user?.email) === "neo4j.com" &&
      user?.email !== "anon.neoconverse@neo4j.com"
    ) {
  ...
  function getListContratct(List): string {}
