...
export async function ExecuteCypher(
  ...
  if (invokeFromClient) {
    ...
    let { port, host, protocol, database, username, password } = !isSaveConvo
      ? agent.connection
      : agent.convoConnection;
...
async function cypherCleanup(generatedCypher: string) {
  ...
  generatedCypher = (await isCypherQuery(generatedCypher))
    ? generatedCypher
    : "RETURN '" + generatedCypher + "' as response";
  ...
  if (
    !(
      generatedCypher.includes("LEADS_TO") ||
      generatedCypher.includes("NEXT") ||
      generatedCypher.includes("PREVIOUS")
    )
  ) {
...
async function chartPropsCleanup(generatedChartProps: string) {
  let chartConfStr = generatedChartProps
    .replaceAll("```", "")
    .replace("json", "")
    .replace("jsx", "")
    .replace("const option =", "");
  chartConfStr = chartConfStr.toString().trim().startsWith("{")
    ? chartConfStr.toString().trim().replace(";", "")
    : "{" + chartConfStr.toString().trim().replace(";", "");
